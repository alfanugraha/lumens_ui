...
preques_folder<-paste(idx_PreQUES, "_PreQUES_testing_",T1,"_",T2,"_",pu_name,sep="")
...
# planning unit
if (data_pu$RST_DATA=="ref") { #Project aceh disini False
  zone<-ref
...
if(nrow(data_xtab)==0){
  ...
  lu.db<-dbReadTable(DB, c("public", data_xtab$TBL_DATA)) ##tidak ada data
}
...
chg_only<-chg_only[c(1,3,2)] #mengurutkan kolom
