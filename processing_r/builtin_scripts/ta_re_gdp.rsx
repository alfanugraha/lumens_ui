...
work_dir<-paste(dirname(proj.file), "/TA/GDPScenario", idx_TA_regeco, sep="")
dir.create(work_dir, mode="0777")
setwd(work_dir)

#READ INPUT FILE
list_of_data_lut<-dbReadTable(DB, c("public", "list_of_data_lut"))
row_gdp_scen <- list_of_data_lut[which(list_of_data_lut$TBL_NAME==GDP_scenario_file),]
GDP_scenario<- list_of_data_lut<-dbReadTable(DB, c("public", row_gdp_scen$TBL_DATA))
...
# ==== Report 0. Cover=====
rtffile <- RTF("TA-GDP_change_report.doc", font.size=11, width = 8.267, height = 11.692, omi = c(0,0,0,0))
# INPUT
